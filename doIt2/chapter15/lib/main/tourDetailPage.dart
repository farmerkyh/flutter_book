...
class _TourDetailPage extends State<TourDetailPage> {
  ...
  void initState() {
    ...
    _GoogleMapCamera = CameraPosition(
      target: LatLng(double.parse(widget.tourData.mapy.toString()), double.parse(widget.tourData.mapx.toString())),
      zoom: 16,
    );
    MarkerId markerId = MarkerId(widget.tourData.hashCode.toString());
    marker = Marker(
        position: LatLng(double.parse(widget.tourData.mapy.toString()), double.parse(widget.tourData.mapx.toString())),
        flat: true,
        markerId: markerId);
                        ...
                        child: Container(
                            ...
                            decoration: BoxDecoration(
                                ...
                                image: DecorationImage(
                                    fit: BoxFit.fill,
                                    image: widget.tourData.imagePath == 'null' || widget.tourData.imagePath == null
                          ...
                          ElevatedButton(
                              onPressed: () {
                                Review review = Review(
                                    widget.id, _reviewTextController.value.text, DateTime.now().toIso8601String());
                                widget.databaseReference
                                    .child('tour')
                                    .child(widget.tourData.id.toString())
                                    .child('review')
            ...
            ElevatedButton(
              onPressed: () {
                DisableInfo info = DisableInfo(
                    widget.id, disableCheck1.floor(), disableCheck2.floor(), DateTime.now().toIso8601String());
                widget.databaseReference
                    .child("tour")
                    .child(widget.tourData.id.toString())
                    .set(info.toJson())
